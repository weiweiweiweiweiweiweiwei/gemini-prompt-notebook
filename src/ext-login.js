/**
 * 外掛用 Google 登入的最後一步（content script，只在網頁版的 ext-login.html 上執行）。
 *
 * Google 登入完，網址會變成 ext-login.html?code=…，這裡把 code 交給外掛的背景程式，
...
(() => {
  ...
  if (err) { say('Google 登入沒有完成：' + err + '。請回到面板再試一次。', 'bad'); return; }
  if (!code) { say('請從外掛面板的「用 Google 帳號登入」開始。', 'bad'); return; }

  say('正在完成登入…', 'busy');
  chrome.runtime.sendMessage({ type: 'gpn-cloud', op: 'googleFinish', code }, (r) => {
